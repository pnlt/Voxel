...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Character1 : CharacterBehaviour
	{
		...
		protected override void Update()
		{
			...
			switch (aiming)
			{
				...
				case true when !wasAiming:
					...
					bodyAnimator.SetBool("IsAim", true);
					break;
				//Just Stopped.
				case false when wasAiming:
					equippedWeaponScope.OnAimStop();
					bodyAnimator.SetBool("IsAim", false);
					break;
			}
		...
		private void UpdateAnimator()
		{
			...
			//Locomotion animation handler
			bodyAnimator.SetFloat("Horizontal", axisMovement.x);
			bodyAnimator.SetFloat("Vertical", axisMovement.y);
			var move = bodyAnimator.GetFloat("Vertical");

			/*if (move > 0)
				bodyAnimator.gameObject.transform.localPosition = new Vector3(0, 0, -0.3f);
			else
				bodyAnimator.gameObject.transform.localPosition = new Vector3(0, 0, -0.2f);*/
			...
			bodyAnimator.SetBool("canRun", running);
		...
		private void Fire()
		{
			...
			equippedWeapon.FireServerRpc(aiming ? equippedWeaponScope.GetMultiplierSpread() : 1.0f);
